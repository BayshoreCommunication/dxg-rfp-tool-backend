...
export const createGetOwnedVendorSubmissionDetail =
  (
    repository: VendorResponseReadRepository,
    documentUrlSigner?: VendorDocumentUrlSigner,
  ) =>
  async (input: { responseId: string; ownerUserId: string }) => {
    const response = await repository.markOwnedRead(input);
    if (!response) return { kind: "not_found" as const };
    const timeline = await repository.getOwnedSubmissionTimeline(input);
    if (!timeline) return { kind: "not_found" as const };
