...
const htmlDocument = (title: string, body: string, reportManifest: Record<string, unknown>) => `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>${style}</style></head><body><main class="page">${body}<section class="section"><h2>Report provenance</h2><p class="small muted">This report is a deterministic projection of a frozen comparison. Any ranking is an advisory result of planner-defined eligibility gates and completed human rubric scores; it is not an award decision.</p><pre class="manifest">${escapeHtml(JSON.stringify(reportManifest, null, 2))}</pre></section></main></body></html>`;
