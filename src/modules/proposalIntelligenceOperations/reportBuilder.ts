...
const htmlDocument = (title: string, body: string, reportManifest: Record<string, unknown>) => `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>${style}</style></head><body><main class="page">${body}<section class="section"><h2>Report provenance</h2><p class="small muted">This report is a deterministic projection of a frozen comparison. Any ranking is an advisory result of planner-defined eligibility gates and completed rubric scores, including labeled evidence-derived automation where used; it is not an award decision.</p><pre class="manifest">${escapeHtml(JSON.stringify(reportManifest, null, 2))}</pre></section></main></body></html>`;
...
const recommendation = (workspace: any) => {
  const value = workspace.recommendation ?? workspace.snapshot?.recommendation;
  if (!value) return `<section class="section"><h2>Advisory recommendation</h2><p class="muted">No recommendation policy result is stored for this historical snapshot.</p></section>`;
  const reasons = Array.isArray(value.confidenceReasons) && value.confidenceReasons.length ? `<ul>${value.confidenceReasons.map((reason: string) => `<li>${escapeHtml(label(reason))}</li>`).join("")}</ul>` : "";
  return `<section class="section"><h2>Advisory recommendation</h2><p><span class="badge">${escapeHtml(label(value.status))}</span> <span class="badge ${value.confidence === "low" ? "danger" : value.confidence === "medium" ? "warn" : "good"}">${escapeHtml(label(value.confidence))} confidence</span></p><p>${escapeHtml(value.rationale)}</p>${reasons}<table><thead><tr><th>Rank</th><th>Vendor</th><th>Human score</th><th>Eligible</th><th>Evaluator count</th><th>Max spread</th></tr></thead><tbody>${value.ranking.map((item: any) => `<tr><td>${escapeHtml(item.rank ?? "-")}</td><td>${escapeHtml(item.vendorLabel)}</td><td>${escapeHtml(Number(item.score).toFixed(2))}</td><td>${item.eligible ? "Yes" : "No"}</td><td>${escapeHtml(item.evaluatorCount ?? "-")}</td><td>${escapeHtml(item.maxCriterionSpread ?? "-")}</td></tr>`).join("")}</tbody></table><p class="small muted">Reviewers retain the final vendor decision.</p></section>`;
};
...
const buildHtml = (input: ReportInput, reportManifest: Record<string, unknown>) => {
  let title = "Executive comparison report", content = `${summaryCards(input.workspace)}${recommendation(input.workspace)}${vendorOverview(input.workspace)}${risks(input.workspace)}${evaluations(input.workspace)}${decisions(input.workspace)}${clarificationsHtml(input.clarifications)}`;
...
const buildPdf = async (input: ReportInput, reportManifest: Record<string, unknown>) => {
  ...
  text("This deterministic report presents persisted evidence and human evaluation state. Any ranking is advisory and is not an award decision.", { size: 11, gap: 18 });
  const advisory = input.workspace.recommendation ?? input.workspace.snapshot?.recommendation;
  if (advisory) { heading("Advisory recommendation"); text(advisory.rationale, { size: 11 }); text(`Confidence: ${label(advisory.confidence)}${advisory.confidenceReasons?.length ? ` - ${advisory.confidenceReasons.map(label).join(", ")}` : ""}`, { size: 9, gap: 10 }); }
