...
test("CSV parser preserves quoted commas, header meaning, and row coordinates",async()=>{const result=await deterministicParser.parse(Buffer.from('item,price\n"LED wall, 4K",1200'),"text/csv");assert.equal(result.fragments[1].content,"Column 1 (item): LED wall, 4K\nColumn 2 (price): 1200");assert.deepEqual(result.fragments[1].coordinates,{row:2,columnStart:1,columnEnd:2});});
