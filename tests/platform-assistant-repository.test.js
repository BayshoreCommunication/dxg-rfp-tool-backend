...
test("migration creates tenant-isolated assistant storage and widens the attempt ledger", () => {
  const up = fs.readFileSync(
    path.join(root, "migrations/postgres/033_platform_assistant.up.sql"),
    "utf8",
  );
  const down = fs.readFileSync(
    path.join(root, "migrations/postgres/033_platform_assistant.down.sql"),
    "utf8",
  );
...
test("intent migration stores bounded privacy-safe routing metadata", () => {
  const up = fs.readFileSync(
    path.join(root, "migrations/postgres/034_platform_assistant_intent.up.sql"),
    "utf8",
  );
  const down = fs.readFileSync(
    path.join(root, "migrations/postgres/034_platform_assistant_intent.down.sql"),
    "utf8",
  );
  for (const required of [
