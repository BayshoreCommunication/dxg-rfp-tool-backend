...
const requestContext = (req: AuthRequest) => {
  if (!contextEnabled())
    throw new ProposalContextError(
      ...
      "Proposal context extraction is disabled outside the approved Slice 2D test environment.",
      503,
    );
...
const handle = (res: Response, error: unknown) => {
  const known = error instanceof ProposalContextError,
  ...
  res
    ...
    .json({
      ...
      title: known ? error.message : "Proposal context operation failed",
      status,
      code,
    });
};
...
export const createProposalContextJob = async (
  ...
  try {
    const ctx = requestContext(req),
      input = contextInput(req.body as Record<string, unknown>),
      result = await proposalContextRepository.create({
        ...ctx,
        ...input,
        live: req.path.includes("live-context-jobs"),
    ...
    res
      ...
      .json({
        data: {
          ...
          resultUrl: `/api/v1/proposals/${req.params.proposalId}/context-runs/${result.runId}`,
        },
      });
  } catch (error) {
    handle(res, error);
...
export const getProposalContextRun = async (
  req: AuthRequest,
  res: Response,
) => {
  try {
    const ctx = requestContext(req);
    res.json({
      data: await proposalContextRepository.read({
        ...ctx,
        proposalMongoId: proposalId(req.params.proposalId),
...
export const getLatestProposalContextRun = async (
  req: AuthRequest,
  res: Response,
) => {
  try {
    const ctx = requestContext(req),
      proposalMongoId = proposalId(req.params.proposalId),
      latestRunId = await latestProposalContextRunId({
        ...ctx,
        proposalMongoId,
      });
    res.json({
