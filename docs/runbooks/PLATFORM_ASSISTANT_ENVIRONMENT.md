...
The last status must show migrations `033_platform_assistant` through
`043_assistant_retention_privacy` as `applied`. Do not run `rollback` as part
...
2. Apply and verify migrations `033_platform_assistant` through
   `043_assistant_retention_privacy`.
