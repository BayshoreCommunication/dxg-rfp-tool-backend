...
Migration `033_platform_assistant` owns:
...
Migration `034_platform_assistant_intent` adds the selected intent, router
...
Migration `039_assistant_feedback` adds bounded response-kind,
...
Migration `040_assistant_product_analytics` adds an append-oriented,
...
Migration `041_assistant_quality_indexes` adds organization/date/version and
...
Migration `042_governed_assets` introduces one tenant-scoped governance
